...
-   **Web-Scraping**, also eine automatisierte Methode zum Extrahieren der Textinformationen von der Webseite News4teachers. Eine Einführung in das Thema Web-Scraping mit R bieten @Wickham2023-hx (Kap. 25[^2]).
...
Wir sehen z.B. in der Zeile 1297 (@fig-html1), dass der Titel des Beitrages ein `h1`-HTML-Element[^7] ist (header 1: Überschrift erster Ebene):

[^7]: <https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Heading_Elements>
...
zusammenfassung <- c("„Schämt Euch!“ – so heißt es auf einem Transparent...", "...", "...")
text <- c("Der offizielle Beitrag Deutschlands fällt dünn aus...", "...", "...")
usw. <- c("...", "...", "...")

data.frame(id, datum, ort, titel, zusammenfassung, text, usw.) |>
  flextable() |>
  autofit() |>
  bold(part = "header")
```
...
In @fig-html1 sehen wir, das die ID des Beitrages (`id="post-132285"`) ein Attribut[^8] eines HTML-Elements ist (HTML-Element: `article`[^9]):

[^8]: <https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/id>
...
%%| fig-cap: Hierarchische Schachtelung der HTML-Elemente `article` und `time`
%%{init: {'theme':'forest'}}%%
flowchart TB
  article --> time
```
...
#### Ort der Berichterstattung {.unnumbered .unlisted}
...
[^12]: <https://developer.mozilla.org/en-US/docs/Web/HTML/Element/p?retiredLocale=de>
...
[^13]: Bei so einer kryptischen Formel (`"[^.]+"` bzw. `"\\.[\\s](.*)"`) handelt es sich um eine so genannte "*regualar expression*" (*regex*). Eine Einführung in diese Thematik findet man hier: <https://r4ds.hadley.nz/regexps> [@Wickham2023-hx, Kapitel 16].
...
#### Zusammenfassung des Beitrages {.unnumbered .unlisted}
...
Somit erhalten wir die Zusammenfassung mit der Ortsangabe inkl. Punkt (`GENF.`). Nun wollen wir die überflüssige Ortsangabe entfernen und nur die eigentliche Zusammenfassung beibehalten. Dies erreichen wir mit dem Befehl `str_extract("\\.[\\s](.*)")`. Die kryptische Formel `"\\.[\\s](.*)"`[^14] hat folgende Bedeutung:

[^14]: Bei so einer kryptischen Formel (`"[^.]+"` bzw. `"\\.[\\s](.*)"`) handelt es sich um eine so genannte "*regualar expression*" (*regex*). Eine Einführung in diese Thematik findet man hier: <https://r4ds.hadley.nz/regexps> [@Wickham2023-hx, Kapitel 16].

-   `\\.` Suche und extrahiere Zeichen nach dem ersten Punkt
